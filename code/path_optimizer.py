...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        distance_to_goal = np.sqrt(xf ** 2 + yf ** 2)

        # Initial assumption
        initial_parameters = [0.0, 0.0, distance_to_goal]

        # Setting bounds
        max_bound = sys.float_info.max
        parameter_bounds = [[-0.5, 0.5], [-0.5, 0.5], [distance_to_goal, max_bound]]
        ...
        return self.sample_spiral(result.x)

    def thetaf(self, a, b, c, d, s):
        thetas = a * s + 0.5 * b * s ** 2 + (1 / 3) * c * s ** 3 + 0.25 * d * s ** 4
        return thetas
    ...
    def sample_spiral(self, p):
        ...
        b = (2 * p[1] - p[0]) / p[2]
        c = (3 * p[1] - 2 * p[0]) / p[2] ** 2
        d = (p[0] - 3 * p[1] + 2 * p[2]) / p[2] ** 3

        s_values = np.linspace(0.0, p[2], num=1000)
        theta_values = self.thetaf(a, b, c, d, s_values)

        x_points = scipy.integrate.cumtrapz(np.cos(theta_values), s_values, initial=0)
        y_points = scipy.integrate.cumtrapz(np.sin(theta_values), s_values, initial=0)

        return [x_points, y_points, theta_values]---------------------------
